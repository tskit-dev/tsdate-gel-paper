...
pre, dist, post  = figure.subfigures(1, 3).ravel()
axs = [pre.subplots(1), post.subplots(1)]
...
draw_edge(axs[0], rparent_x[0] + offset, rparent_x[0], rparent_x[1] + offset, rroot_x[1], rparent_y[1], rroot_y[0], s=0.5) # right parent -> right root
...
axs[0].text(rroot_x[0]/2 + rroot_x[1]/2, lroot_y[1], "ancestral haplotype", ha="center", va="bottom", color="gray")
axs[0].text(sample_x[0]/2 + sample_x[1]/2, sample_y[1] - 0.75, "sample haplotype", ha="center", va="top", color="gray")
axs[0].text(rroot_x[0] + 6, rroot_y[1] - 5.00, "edge", ha="center", va="top", color="blue", alpha=0.3, rotation=75)
axs[0].text(6, 13.5, "mutation", ha="center", va="bottom", color="blue")
axs[0].set_xlim(sample_x[0] - 3 * offset, sample_x[1] + 2 * offset)
axs[0].set_ylim(sample_y[0] - 0.1 * offset, max(rroot_y[1], lroot_y[1]) + offset)
axs[0].spines['top'].set_visible(False)
axs[0].spines['right'].set_visible(False)
axs[0].spines['bottom'].set_visible(False)
axs[0].set_yticks(np.array([0, lparent_y[0], lroot_y[0]]) + 0.25)
axs[0].set_yticklabels([0, 0.5, 1.0], color="white")
axs[0].set_ylabel("Uncalibrated time")
axs[0].set_xticklabels([" ", " "])
axs[0].tick_params(axis="x", color="white")
axs[0].set_xlabel(" ")
axs[0].text(0.03, 0.99, "tsinfer", ha="left", va="top", size=12, transform=axs[0].transAxes)
...
axs[0].text(rroot_x[1] + nl, rroot_y[0] - 0.3, "d", ha="left", color="gray", size=10)
...
# --- PANEL 2: dated tree sequence --- #

# use mean after all iterations (previous fig versions used first iteration means)
final_mean = (ep_posteriors[-1][:, 0] + 1) / ep_posteriors[-1][:, 1]
mean[1:] = final_mean[1:]

scaling = max(lroot_y[1], rroot_y[1]) / mean.max()
nodes_time = mean * scaling
sample_y = [0, 0.5]
...
draw_edge(axs[1], lparent_x[0], lparent_x[0] - offset, lparent_x[1], lparent_x[1] - offset, sample_y[1], lparent_y[0], s=0.5) # sample -> left parent
draw_edge(axs[1], rparent_x[0], rparent_x[0] + offset, rparent_x[1], rparent_x[1] + offset, sample_y[1], rparent_y[0], s=0.5) # sample -> right parent
draw_edge(axs[1], lroot_x[0] - offset, lroot_x[0] - 2*offset, lroot_x[1] - offset, lroot_x[1] - 2*offset, lparent_y[1], lroot_y[0], s=0.5) # left parent -> left root
draw_edge(axs[1], rroot_x[0] - offset, rroot_x[0], lparent_x[1] - offset, rroot_x[1], lparent_y[1], rroot_y[0], s=0.5) # left parent -> right root
#draw_edge(axs[1], rparent_x[0] + offset, rparent_x[0], rparent_x[1] + offset, rroot_x[1], rparent_y[1], rroot_y[0], s=0.5, color="firebrick", alpha=0.2) # right parent -> right root #<<< with highlight
draw_edge(axs[1], rparent_x[0] + offset, rparent_x[0], rparent_x[1] + offset, rroot_x[1], rparent_y[1], rroot_y[0], s=0.5, alpha=0.1) # right parent -> right root #<<< without highlight

draw_haplotype(axs[1], sample_x[0], sample_x[1], sample_y[0], sample_y[1]) # sample
draw_haplotype(axs[1], lparent_x[0] - offset, lparent_x[1] - offset, lparent_y[0], lparent_y[1]) # left sample parent
draw_haplotype(axs[1], rparent_x[0] + offset, rparent_x[1] + offset, rparent_y[0], rparent_y[1]) # right sample parent
draw_haplotype(axs[1], lroot_x[0] - offset * 2, lroot_x[1] - offset * 2, lroot_y[0], lroot_y[1]) # left root
draw_haplotype(axs[1], rroot_x[0] - offset * 0, rroot_x[1] - offset * 0, rroot_y[0], rroot_y[1]) # left root

for xp, yp in zip(mut_x, mut_y):
    if yp > 1: # upper edges
        if xp < lroot_x[1] / sample_x[1]:
            draw_mutation_root(axs[1], sample_x, sample_y, lparent_x, lparent_y, -offset, lroot_x, lroot_y, -2*offset, s=0.5, x_pos=xp, y_pos=yp-1, markersize=3)
        elif xp > lroot_x[1] / sample_x[1] and xp < rparent_x[0] / sample_x[1]:
            draw_mutation_root(axs[1], sample_x, sample_y, lparent_x, lparent_y, -offset, rroot_x, rroot_y, 0, s=0.5, x_pos=xp, y_pos=yp-1, markersize=3)
        else:
            draw_mutation_root(axs[1], sample_x, sample_y, rparent_x, rparent_y, offset, rroot_x, rroot_y, 0, s=0.5, x_pos=xp, y_pos=yp-1, markersize=3)
    else: # lower edges
        if xp < lparent_x[1] / sample_x[1]:
            draw_mutation_parent(axs[1], sample_x, sample_y, lparent_x, lparent_y, -offset, s=0.5, x_pos=xp, y_pos=yp, markersize=3)
        else:
            draw_mutation_parent(axs[1], sample_x, sample_y, rparent_x, rparent_y, offset, s=0.5, x_pos=xp, y_pos=yp, markersize=3)

axs[1].set_title("C", ha="right", x=-0.13, size=14)
axs[1].set_xlim(sample_x[0] - 3 * offset, sample_x[1] + 2 * offset)
axs[1].set_ylim(sample_y[0] - 0.1 * offset, max(rroot_y[1], lroot_y[1]) + offset)
axs[1].set_ylabel("Generations in past")
axs[1].set_yticks(np.array([0, 0.5, 1]) * scaling)
axs[1].set_yticklabels(np.array([0, 0.5, 1]))
axs[1].set_xlabel(" ")
axs[1].set_xticklabels([" ", " "])
axs[1].text(-6.2, 1.4 * scaling, r"$\times 10^3$", va="top", ha="right", fontsize=9)
axs[1].tick_params(axis="x", color="white")
axs[1].spines['top'].set_visible(False)
axs[1].spines['right'].set_visible(False)
axs[1].spines['bottom'].set_visible(False)
axs[1].text(0.03, 0.99, "tsinfer + tsdate", ha="left", va="top", size=12, transform=axs[1].transAxes)
axs[1].text(sample_x[0]/2 + sample_x[1]/2, sample_y[1] - 0.75, "sample haplotype", ha="center", va="top", color="gray")

nl = 0.2
axs[1].text(lparent_x[0] - offset - nl, lparent_y[0], "a", ha="right", color="gray", size=10)
axs[1].text(lroot_x[0] - offset * 2 - nl, lroot_y[0], "c", ha="right", color="gray", size=10)
axs[1].text(rparent_x[1] + offset + nl, rparent_y[0], "b", ha="left", color="gray", size=10)
axs[1].text(rroot_x[1] + nl, rroot_y[0], "d", ha="left", color="gray", size=10)
...
# --- PANEL 3: EP detail as contour plot --- #
#
#levels = np.linspace(0.2, 1.0, 10) # probability isoclines
#
#poly = [(0, 0), (t_max, t_max), (t_max, 0), (0, 0)]
#axs[2].add_patch(Polygon(poly, facecolor="lightgray", fill=True, alpha=0.5))
#axs[2].contour(
#    grid_i, grid_j, logl_cavity, 
#    levels=quantile_to_level(logl_cavity, levels), 
#    colors="dodgerblue",
#)
#axs[2].text(1.0, 0.1, "Posterior without edge", ha="left", va="center", color="dodgerblue", size=10)
#axs[2].contour(
#    grid_i, grid_j, logl_exact, 
#    levels=quantile_to_level(logl_exact, levels), 
#    colors="black",
#)
#axs[2].text(0.05, 0.5, "Edge update", ha="left", va="center", color="black", size=10)
#axs[2].contour(
#    grid_i, grid_j, logl_update, 
#    levels=quantile_to_level(logl_update, levels), 
#    colors="firebrick",
#)
#axs[2].text(0.5, 1.9, "Match moments", ha="left", va="bottom", color="firebrick", size=10)
#axs[2].add_patch(
...
#        connectionstyle="arc3,rad=-.25", arrowstyle=u"->", lw=1)
#)
#axs[2].set_title("B", ha="right", x=-0.1, size=14)
#axs[2].set_xlim(grid_j.min(), grid_j.max())
#axs[2].set_ylim(grid_i.min(), grid_i.max())
#axs[2].set_ylabel("Parent node age (generations)")
#axs[2].set_xlabel("Child node age (generations)")
#axs[2].text(2.48, -0.01, r"$\times 10^3$", va="top", ha="right", fontsize=9)
#axs[2].text(-0.01, 2.45, r"$\times 10^3$", va="top", ha="right", fontsize=9)
#axs[2].text(0.03, 0.99, "Expectation propagation", transform=axs[2].transAxes, ha="left", va="top", color="black", size=12)
#axs[2].spines['top'].set_visible(False)
#axs[2].spines['right'].set_visible(False)


# --- PANEL 4: EP vs true posteriors --- #

#fig, axs = plt.subplots(4, 1, figsize=(4, 4), sharex=True, constrained_layout=True)
dist_axs = dist.subplots(4, 1, sharex=True)
grid = np.linspace(0, 2.5, 100)
node_labels = ["a", "b", "c", "d"]
for i, (post, true) in enumerate(zip(ep_posteriors[-1][1:], true_posteriors[1:])):
    dist_axs[i].hist(true, density=True, color="firebrick", bins=50, alpha=0.5, label="True posterior")
    shape, rate = post
    shape += 1
    dens = scipy.stats.gamma.pdf(grid, shape, scale=1/rate)
    dist_axs[i].plot(grid, dens, alpha=1, color="black", label="EP approximation")
    dist_axs[i].set_xlim(0, grid.max())
    dist_axs[i].spines['top'].set_visible(False)
    dist_axs[i].spines['right'].set_visible(False)
    dist_axs[i].set_yticks([0, 1])
    dist_axs[i].set_xticks(np.array([0.0, 0.5, 1.0, 1.5, 2.0]))
    dist_axs[i].text(0.02, 0.99, node_labels[i], ha="left", va="top", size=10, transform=dist_axs[i].transAxes)
dist_axs[0].legend(frameon=False)
dist_axs[-1].text(2.5, -0.15, r"$\times 10^3$", va="top", ha="right", fontsize=9)
dist.suptitle("B", ha="right", x=0.06, size=14)
dist.supylabel("Posterior density", size=10)
dist.supxlabel("Node age (generations)", size=10)


# ---------------------------------------- #


plt.savefig("../figures/algorithm_schematic.pdf")
